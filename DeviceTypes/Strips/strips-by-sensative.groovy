...
def sensorValueEvent(value) {
  ...
  if (invertOutput) {value = value ^ 0x1}
	if (value) {
...
def zwaveEvent(physicalgraph.zwave.commands.notificationv3.NotificationReport cmd) {
  sendEvent(name:"debug", value: "notificationv3", displayed: true)
	def result = []
...
def zwaveEvent(physicalgraph.zwave.commands.wakeupv1.WakeUpNotification cmd) {
	...
	def temp = device.currentValue("needUpdate")
...
def buildInterval() {
  ...
  def intervalList = []
   ...
   intervalList << [ 2880000 : "1 month (30 days)" ]
   intervalList << [ 5760000 : "2 months (60 days)" ]
   intervalList << [ 8736000 : "3 months (91 days)" ]
   intervalList << [ 11616000 : "4 months (121 days)" ]
   intervalList << [ 17520000 : "Half year" ]
   intervalList << [ 35040000 : "1 year" ]
}
...
def zwaveEvent(physicalgraph.zwave.commands.versionv1.VersionReport cmd) {
 	def appversion = cmd.applicationVersion.toString() + "." + cmd.applicationSubVersion.toString()
 	def zprotoversion = cmd.zWaveProtocolVersion.toString() + "." + cmd.zWaveProtocolSubVersion.toString()
	updateDataValue("zWaveLibraryType", cmd.zWaveLibraryType.toString())
    updateDataValue("ApplicationVersion", appversion)
	updateDataValue("zWaveProtocolVersion", zprotoversion)
}
